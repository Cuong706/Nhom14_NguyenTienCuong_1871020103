...
def dfs(maze, order='default'):
    """
    Thuật toán DFS sử dụng Stack (Ngăn xếp) LIFO
    ...
    # Stack lưu (vị trí hiện tại, đường đi đến đó)
    stack = [(start, [start])]
    ...
    print(f"\n{'='*50}")
    print(f"🚀 DFS BẮT ĐẦU - Thứ tự: {order}")
    ...
    while stack:
        ...
        if current == goal:
            print(f"\n✅ DFS: TÌM THẤY ĐÍCH sau {step} bước duyệt!")
    ...
    print(f"\n❌ DFS: KHÔNG tìm thấy đường đi đến đích!")
